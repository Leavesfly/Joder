...
public class AgentsManager {
    ...
    @Inject
    public AgentsManager(@WorkingDirectory String workingDir) {
    ...
    private void loadAgents() {
        ...
        // 2. 加载全局代理
        loadAgentsFromDirectory(globalAgentsPath, AgentConfig.AgentType.USER);
        
        // 3. 加载项目代理
        loadAgentsFromDirectory(projectAgentsPath, AgentConfig.AgentType.PROJECT);
     ...
     * 从目录加载代理
     */
    private void loadAgentsFromDirectory(Path directory, AgentConfig.AgentType type) {
     ...
     * 根据类型获取代理列表
     */
    public List<AgentConfig> getAgentsByType(AgentConfig.AgentType type) {
    ...
    public void createAgent(AgentConfig agent, boolean isProject) throws IOException {
        ...
        agent.setType(isProject ? AgentConfig.AgentType.PROJECT : AgentConfig.AgentType.USER);
    ...
    public void updateAgent(AgentConfig agent) throws IOException {
        ...
        if (existing.getType() == AgentConfig.AgentType.BUILTIN) {
    ...
    public void deleteAgent(String name) throws IOException {
        ...
        if (agent.getType() == AgentConfig.AgentType.BUILTIN) {
